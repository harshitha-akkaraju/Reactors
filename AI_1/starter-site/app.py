...
# sends image to Computer Vision and prepares the results
...
def extract_text_from_image(image, client):
    try:
        ...
        if len(result.regions) == 0:
            lines.append("Picture does not contain text to translate.")
        else:
            for line in result.regions[0].lines:
                # concatenates all the words in a line
...
def translate_text(lines, target_language, key, region):
    ...
    try:
        ...
        return 'Error calling Translator Text API: ' + e.strerror
